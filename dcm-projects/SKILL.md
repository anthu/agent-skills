...
2. **Load**: [reference/project-structure.md](reference/project-structure.md) - Project structure and manifest.yml configuration
3. **Load**: [reference/cli-reference.md](reference/cli-reference.md) - All DCM CLI commands and workflows
...
### MULTI_PROJECT Intent - User wants hierarchical or multiple related DCM projects

**Trigger phrases**: "multiple projects", "team projects", "platform admin", "hierarchical dcm", "delegate deployment", "platform infrastructure", "team-level dcm"

**→ ✋ MUST Load**: [multi-project/SKILL.md](multi-project/SKILL.md) — DO NOT create multi-project structures until this sub-skill is loaded.

### BULK_IMPORT Intent - User wants to import many objects from existing schema

**Trigger phrases**: "import schema", "bulk import", "discover objects", "scan database", "migrate to dcm", "cross-account", "import all tables", "schema discovery"

**→ ✋ MUST Load**: [bulk-import/SKILL.md](bulk-import/SKILL.md) — DO NOT run discovery queries until this sub-skill is loaded.
...
Start Session
    ↓
MANDATORY: Load reference documents (syntax.md, project-structure.md, cli-reference.md)
    ↓
...
Detect User Intent
    ...
    │
    ├─→ BULK_IMPORT → ✋ MUST Load bulk-import/SKILL.md BEFORE discovery
    │   (Triggers: "import schema", "bulk import", "discover objects", "scan database")
    │   → Use discovery script → Categorize objects → Generate definitions
    │   → For cross-account: export/import via stage
    │
    ├─→ MULTI_PROJECT → ✋ MUST Load multi-project/SKILL.md BEFORE setup
    │   (Triggers: "multiple projects", "team projects", "platform admin", "hierarchical")
    │   → Platform project first → Grant CREATE DCM PROJECT → Team projects
    │   → See reference/multi-project-patterns.md for templates
    │
...
| [multi-project/SKILL.md](multi-project/SKILL.md) | Hierarchical multi-project patterns         | MULTI_PROJECT intent          |
| [bulk-import/SKILL.md](bulk-import/SKILL.md)       | Bulk schema discovery and import            | BULK_IMPORT intent            |

**Note:** The IMPORT_EXISTING workflow (adopting existing objects) is documented in [modify-project/SKILL.md](modify-project/SKILL.md) and in [Workflow 5: Adopting Existing Objects](#workflow-5-adopting-existing-objects). For **bulk** imports of entire schemas, use the [bulk-import/SKILL.md](bulk-import/SKILL.md) sub-skill instead.
...
**Note:** For multi-project architectures (platform + team projects, hierarchical deployments, delegated privileges), load the **multi-project** skill. See also [reference/multi-project-patterns.md](reference/multi-project-patterns.md) for templates.
...
- [Project Structure Guide](reference/project-structure.md)
