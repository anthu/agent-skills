...
## Special Case: Two-Phase Deployment for DBT Projects

### ⚠️ Transaction Isolation with DEFINE DBT PROJECT

When a DCM project includes `DEFINE DBT PROJECT` **and** creates new schemas in the same deployment, you must use **two-phase deployment**.

**Why:** `DEFINE DBT PROJECT` spawns a separate dbt subprocess that opens its own database connection. This subprocess **cannot see uncommitted schema changes** from the same DCM transaction, causing "Failed to use schema" errors.

### When Two-Phase Deployment is Required

- **Required:** First deployment of a project that creates schemas AND defines a dbt project
- **Not required:** Subsequent deployments (schemas already exist and are committed)
- **Not required:** Projects without `DEFINE DBT PROJECT`

### Recommended File Structure

Separate definitions into two files:
...
```bash
# Temporarily exclude dbt.sql
mv sources/definitions/dbt.sql sources/definitions/dbt.sql.skip

# Deploy infrastructure (creates and commits schemas)
snow dcm plan <identifier> -c <connection> --target <target> --save-output
# Review plan...
snow dcm deploy <identifier> -c <connection> --target <target> --alias "phase1-infra"
```
...
snow dcm plan <identifier> -c <connection> --target <target> --save-output
# Review plan...
snow dcm deploy <identifier> -c <connection> --target <target> --alias "phase2-dbt"
```
...
```
Failed to use schema <SCHEMA_NAME>
```

And the schema is defined in the same DCM project with `DEFINE SCHEMA`, you've hit the transaction isolation issue. Use the two-phase deployment workflow above.
