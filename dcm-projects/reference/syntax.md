...
> **Note**: The examples below demonstrate Jinja patterns. Variable values come from the `templating` section in `manifest.yml` — resolved by merging `templating.defaults` with the active `templating.configurations` entry selected by the target's `templating_config`. See `reference/project-structure.md` for details.
...
> **⚠️ Inline Macro Scope**: Macros defined inline within a definition file are **only accessible within that same file**. They cannot be imported or referenced from other definition files. For macros that need to be shared across multiple files, place them in `sources/macros/` — macros in that directory are accessible from all definition files. See `reference/project-structure.md` for details on the macros directory.
...
DCM supports embedding dbt projects using the `DEFINE DBT PROJECT` statement. This creates a Snowflake-native dbt project object that can be executed via `EXECUTE DBT PROJECT`.

### DBT Project Definition Syntax

```sql
DEFINE DBT PROJECT database_name.schema_name.project_name
  FROM 'relative/path/to/dbt/project'
...
|----------|-------------|
| `FROM` | Relative path to dbt project directory (from DCM project root) |
| `DEFAULT_TARGET` | The dbt target to use by default (e.g., 'dev', 'prod') |

### Example

```sql
DEFINE DBT PROJECT MY_DB{{env_suffix}}.CORE.DBT_MY_PROJECT
  FROM 'sources/dbt'
  DEFAULT_TARGET = '{{dbt_env}}';
```

### ⚠️ CRITICAL: Transaction Isolation Limitation

**`DEFINE DBT PROJECT` spawns a separate dbt subprocess that opens its own database connection.** This subprocess **cannot see uncommitted schema changes** from the same DCM transaction.

**Problem scenario:**
```sql
-- infrastructure.sql
DEFINE SCHEMA MY_DB.CORE;  -- Schema created in transaction

-- dbt.sql (same deployment)
DEFINE DBT PROJECT MY_DB.CORE.DBT_PROJECT  -- FAILS: dbt can't see CORE schema
  FROM 'sources/dbt'
  DEFAULT_TARGET = 'dev';
```
...
**Error message:**
```
Failed to use schema CORE
```

**Root cause:** The dbt subprocess queries `INFORMATION_SCHEMA.SCHEMATA` to validate the target schema exists, but it cannot see schemas created within the uncommitted DCM transaction.

### Solution: File Separation Pattern

Separate your DCM definitions into two files:

1. **`infrastructure.sql`** - Schemas, roles, grants (everything the dbt project depends on)
2. **`dbt.sql`** - Only the `DEFINE DBT PROJECT` statement

```
sources/definitions/
├── infrastructure.sql   # Schemas, roles, grants
└── dbt.sql              # DEFINE DBT PROJECT only
```

**infrastructure.sql:**
```sql
DEFINE SCHEMA MY_DB{{env_suffix}}.INPUT;
DEFINE SCHEMA MY_DB{{env_suffix}}.CORE;
DEFINE SCHEMA MY_DB{{env_suffix}}.ACC;

DEFINE ROLE MY_PROJECT_WRITER;
-- ... grants ...
```

**dbt.sql:**
```sql
-- IMPORTANT: Separated due to transaction isolation.
-- dbt subprocess cannot see uncommitted schema changes.
-- Deploy infrastructure.sql first, then redeploy with this file.
...
When creating a new DCM project with `DEFINE DBT PROJECT`:

**Phase 1 - Infrastructure:**
```bash
# Temporarily exclude dbt.sql (rename or move it)
mv sources/definitions/dbt.sql sources/definitions/dbt.sql.skip

# Deploy infrastructure only
snow dcm plan MY_DB.PUBLIC.MY_PROJECT -c connection --target DEV --save-output
snow dcm deploy MY_DB.PUBLIC.MY_PROJECT -c connection --target DEV
```

**Phase 2 - DBT Project:**
```bash
# Restore dbt.sql
mv sources/definitions/dbt.sql.skip sources/definitions/dbt.sql

# Deploy again (dbt can now see committed schemas)
snow dcm plan MY_DB.PUBLIC.MY_PROJECT -c connection --target DEV --save-output
snow dcm deploy MY_DB.PUBLIC.MY_PROJECT -c connection --target DEV
```

**For subsequent deployments:** If schemas already exist, no two-phase deployment needed—a single deployment works.
