...
### Definition File Ordering

DCM processes SQL files in **alphabetical order** within `sources/definitions/`. Use numeric prefixes to control execution order when dependencies exist:

```
sources/definitions/
├── 01_infrastructure.sql    # Schemas, warehouses (must exist first)
├── 02_tables.sql            # Tables (need schemas)
├── 03_dbt.sql               # DBT projects (need schemas committed)
└── 04_access.sql            # Grants (need objects to grant on)
```

**When to use prefixes:**
- `DEFINE DBT PROJECT` requires schemas to be committed (not just defined in the same transaction)
- Grants require objects to exist
- Role hierarchy requires parent roles to exist

**Common pattern for Data Products:**
- `01_infrastructure.sql` - schemas, roles, grants
- `02_dbt.sql` - dbt project definition
